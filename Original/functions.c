...
void insertTrie(trieNode* node, char* word/*, int textID*/){
	trieNode* temp = node;
			...
			for(int j=0; j<(temp->arraySize); j++){
				if(j>0)
					...
					if(i < (strlen(word)-1)){
						...
						/*temp->children[0]->listPtr = (postingsList*)malloc(sizeof(postingsList));
						if(temp->children[0]->listPtr == NULL)
							return;*/
					}
